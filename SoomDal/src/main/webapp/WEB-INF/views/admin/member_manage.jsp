	...
	<div id="table_wrap" >
		...
		<div id="table_report_wrap" style="display:none;">
			...
			<colgroup>
				...
				<c:forEach items="${report.list}" var="r">
					<tr>
						<td>${r.RDate}</td>
						<td>${r.RWriter }</td>
						<td>${r.RScore }</td>
			...
			<div style="text-align: center; display: inline-block; margin-left: 35%">
				<ul class="pagination">
					<c:if test="${report.prev==true }">
						<li><a href="/dal/admin/report_manage?pageno=${report.startPage-1 }">이전</a></li>
					</c:if>
					<c:forEach begin="${report.startPage }" end="${report.endPage }" var="i">
						<c:choose>
							<c:when test="${report.pageno eq i }">
								<li class="active">
									<a href="/dal/admin/report_manage?pageno=${i }">${i }</a>
								</li>
							</c:when>
							<c:otherwise>
								<li><a href="/dal/admin/report_manage?pageno=${i }">${i }</a>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					<c:if test="${report.next==true }">
						<li><a href="/dal/admin/report_manage?pageno=${report.endPage+1 }">다음</a></li>
					</c:if>
				</ul>
			</div>
		</div>		
