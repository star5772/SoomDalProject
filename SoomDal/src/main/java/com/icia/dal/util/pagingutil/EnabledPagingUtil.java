...
public class EnabledPagingUtil {
	// 페이지당 글의 개수
		...
		// 일반글 페이징, 히트글 페이징
		public static EnabledPage getPage(int pageno, int countOfBoard) {
		...
		return EnabledPage.builder().startPage(startPage).endPage(endPage).startRowNum(startRowNum).endRowNum(endRowNum).isPrev(isPrev).isNext(isNext).pageno(pageno).build();
		}
}
