...
public class AdminPagingUtil {
	...
	public static AdminPage getPage(int pageno, int countOfBoard) {
		...
		return AdminPage.builder().startPage(startPage).endPage(endPage).startRowNum(startRowNum).endRowNum(endRowNum).isPrev(isPrev).isNext(isNext).pageno(pageno).build();
		}
