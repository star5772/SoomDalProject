...
public class AdminToJejaPagingUtil {
	// 페이지당 글의 개수
	...
	// 일반글 페이징, 히트글 페이징
	public static PageToJejaAdmin getPage(int pageno, int countOfBoard) {
		...
		return PageToJejaAdmin.builder().startPage(startPage).endPage(endPage).startRowNum(startRowNum).endRowNum(endRowNum).isPrev(isPrev).isNext(isNext).pageno(pageno).build();
		}
