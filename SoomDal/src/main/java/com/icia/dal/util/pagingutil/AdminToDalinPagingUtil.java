...
public class AdminToDalinPagingUtil {
	// 페이지당 글의 개수
	...
	// 일반글 페이징, 히트글 페이징
	public static PageToDalinAdmin getPage(int pagene, int countOfBoard) {
		...
		if(countOfBoard%COUNT_OF_BOARD_PER_PAGE == 0)
			countOfPage--;
		if(pagene>countOfPage)
			pagene=countOfPage;
				
		int startRowNum = (pagene-1)*COUNT_OF_BOARD_PER_PAGE +1;
		...
		int blockNo = (pagene-1)/COUNT_OF_BOARD_PER_BLOCK;
		...
		return PageToDalinAdmin.builder().startPage(startPage).endPage(endPage).startRowNum(startRowNum).endRowNum(endRowNum).isPrev(isPrev).isNext(isNext).pagene(pagene).build();
		}
