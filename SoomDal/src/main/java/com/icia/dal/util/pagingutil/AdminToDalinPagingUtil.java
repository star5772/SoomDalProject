...
public class AdminToDalinPagingUtil {
	...
	// 일반글 페이징, 히트글 페이징
	public static PageToDalinAdmin getPage(int pageno, int countOfBoard) {
		...
		return PageToDalinAdmin.builder().startPage(startPage).endPage(endPage).startRowNum(startRowNum).endRowNum(endRowNum).isPrev(isPrev).isNext(isNext).pageno(pageno).build();
		}
