...
public class UseCashPagingUtil {
		...
		public static PageToUseCash getPage(int pageno, int countOfBoard) {
			...
			return PageToUseCash.builder().startPage(startPage).endPage(endPage).startRowNum(startRowNum).endRowNum(endRowNum).isPrev(isPrev).isNext(isNext).pageno(pageno).build();
			}
