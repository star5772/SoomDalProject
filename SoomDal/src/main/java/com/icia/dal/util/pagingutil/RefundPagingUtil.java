...
public class RefundPagingUtil {
	...
	// 일반글 페이징, 히트글 페이징
	public static PageToRefund getPage(int pageno, int countOfBoard) {
		// 페이지의 개수 계산
		...
		return PageToRefund.builder().startPage(startPage).endPage(endPage).startRowNum(startRowNum)
				.endRowNum(endRowNum).isPrev(isPrev).isNext(isNext).pageno(pageno).build();
	}
}
